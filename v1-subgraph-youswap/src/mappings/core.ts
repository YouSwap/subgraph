...
import {
  ...
  ZERO_BD,
  BI_18,
  createLiquiditySnapshot, setUserUsdMined, setUserUsdBurned, setUserUsdSwapped, BLACKHOLE_ADDRESS
} from './helpers'
...
export function handleTransfer(event: Transfer): void {
  ...
  log.warning("handle transfer height: {} hash: {}", [event.block.number.toString(), event.transaction.hash.toHexString()])
...
export function handleSync(event: Sync): void {
  ...
  log.warning("handle sync height: {} hash: {}", [event.block.number.toString(), event.transaction.hash.toHexString()])
...
export function handleMint(event: Mint): void {
  ...
  log.warning("handle mint height: {} hash: {}", [event.block.number.toString(), event.transaction.hash.toHexString()])
  ...
  setUserUsdMined(event.params.sender, amountTotalUSD as BigDecimal)
...
export function handleBurn(event: Burn): void {
  ...
  log.warning("handle burn height: {} hash: {}", [event.block.number.toString(), event.transaction.hash.toHexString()])
  ...
  setUserUsdBurned(event.params.sender, amountTotalUSD as BigDecimal)
...
export function handleSwap(event: Swap): void {
  ...
  log.warning("handle swap height: {} hash: {}", [event.block.number.toString(), event.transaction.hash.toHexString()])
  ...
  if (event.params.to.toHexString() == BLACKHOLE_ADDRESS) {
    let buyBack = BuyBack.load(pair.id)
    if (buyBack == null){
      buyBack = new BuyBack(pair.id)
      buyBack.totalToken = ZERO_BD
      buyBack.totalUSD = ZERO_BD
    }
    buyBack.totalToken = buyBack.totalToken.plus(amount0Out)
    buyBack.totalUSD = buyBack.totalUSD.plus(amount1In)
    buyBack.save()
  }

  setUserUsdSwapped(event.params.sender, swap.amountUSD)
  // update the transaction
